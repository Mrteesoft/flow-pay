...
[[ "$CHAIN_ID" == "84532" ]] || { echo "refusing deployment: RPC chain ID is $CHAIN_ID, expected 84532" >&2; exit 1; }
...
umask 077
cat > "$RUNTIME_DIR/base-sepolia.env" <<ENV
BASE_SEPOLIA_RPC_URL=$RPC_URL
BASE_SEPOLIA_CHAIN_ID=84532
BASE_SEPOLIA_FACTORY_ADDRESS=$FACTORY
BASE_SEPOLIA_OPERATOR_ADDRESS=$OPERATOR
BASE_SEPOLIA_PROXY_CREATION_CODE_HASH=$PROXY_HASH
BASE_SEPOLIA_FACTORY_RUNTIME_CODE_HASH=$FACTORY_HASH
ENV
printf 'Base Sepolia factory deployed: %s\n' "$FACTORY"
printf 'Operator address: %s\n' "$OPERATOR"
printf 'Metadata written to: %s\n' "$RUNTIME_DIR/base-sepolia.env"
