...
[[ -n "$RPC_URL" ]] || { echo "BSC_TESTNET_RPC_URL is required" >&2; exit 1; }
[[ -r "$KEY_FILE" ]] || { echo "deployer key file is not readable: $KEY_FILE" >&2; exit 1; }

# Accept a single raw hex key, or KEY=value / JSON-style quoted values.
...
[[ "$CHAIN_ID" == "97" ]] || { echo "refusing deployment: RPC chain ID is $CHAIN_ID, expected 97" >&2; exit 1; }
...
umask 077
cat > "$RUNTIME_DIR/bsc-testnet.env" <<ENV
BSC_TESTNET_RPC_URL=$RPC_URL
BSC_TESTNET_CHAIN_ID=97
BSC_TESTNET_FACTORY_ADDRESS=$FACTORY
BSC_TESTNET_OPERATOR_ADDRESS=$OPERATOR
BSC_TESTNET_PROXY_CREATION_CODE_HASH=$PROXY_HASH
BSC_TESTNET_FACTORY_RUNTIME_CODE_HASH=$FACTORY_HASH
ENV

printf 'BSC Testnet factory deployed: %s\n' "$FACTORY"
printf 'Operator address: %s\n' "$OPERATOR"
printf 'Metadata written to: %s\n' "$RUNTIME_DIR/bsc-testnet.env"
