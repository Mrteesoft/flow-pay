...
import {moneyFromStableBalances,networkAsset,statusTone,tokenAsset} from "../lib/format";
...
export default async function Overview(){
  ...
  return <>
    ...
    <section className="panel"><div className="panel-head"><h2>Recent payments</h2><a href="/payments">View all</a></div><div className="table-wrap"><table className="table"><thead><tr><th>Payment</th><th>Customer reference</th><th>Amount</th><th>Network</th><th>Status</th></tr></thead><tbody>{payments.map((p:any)=><tr key={p.id}><td><a className="id-link" href={`/payments/${p.id}`}>{p.id}</a></td><td>{p.reference??"—"}</td><td><span className="asset-cell"><img src={tokenAsset(p.asset)} alt=""/><span className="amount-cell">{p.amount} {p.asset}</span></span></td><td><span className="network-cell"><img src={networkAsset(p.chain)} alt=""/>{p.chain==="bsc"?"BNB Smart Chain":"Base"}</span></td><td><span className={`status ${statusTone(p.status)}`}>{p.status.replaceAll("_"," ")}</span></td></tr>)}</tbody></table>{payments.length===0?<div className="empty-note">No payments yet. Create the first test checkout.</div>:null}</div></section>
  </>;
}
