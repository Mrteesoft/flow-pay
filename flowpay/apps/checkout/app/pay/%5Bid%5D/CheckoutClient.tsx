...
import {
  ArrowRightIcon,CheckIcon,ClockIcon,CopyIcon,HeadphonesIcon,
...
export function CheckoutClient({paymentId}:{paymentId:string}){
  ...
  return <main className="checkout-shell">
    ...
    <section className="checkout-card" aria-labelledby="payment-title">
      ...
      {canClaim?<div className="checkout-help-grid">
        <a href={`/claim?payment_id=${encodeURIComponent(payment.id)}`} className="help-tile">
          <div className="round-icon"><LifebuoyIcon/></div>
          <div><strong>Payment problem?</strong><span>If you sent the wrong asset or network, FlowPay can investigate whether recovery is possible.</span><b>Create a claim <ArrowRightIcon/></b></div>
        </a>
        <a href="mailto:support@flowpay.dev" className="help-tile">
          <div className="round-icon"><HeadphonesIcon/></div>
          <div><strong>Need help?</strong><span>Contact support for payment questions that do not require a recovery investigation.</span><b>Contact support <ArrowRightIcon/></b></div>
        </a>
      </div>:null}
